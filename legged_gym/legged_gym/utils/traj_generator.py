...
class TrajGenerator():
    def __init__(self, num_envs, episode_dur, num_verts, device, dtheta_max,
                 speed_min, speed_max, accel_max, sharp_turn_prob, sharp_turn_angle=np.pi):
        ...
        self._sharp_turn_angle = np.clip(sharp_turn_angle, 0.0, np.pi)
        ...
        env_ids = torch.arange(self.get_num_envs(), dtype=torch.long)
