...
class MotionLib:
    def __init__(self, motion_file, mapping_file, dof_names, fps, device, window_length, ratio_random_range, thresh_robot2chair):
        self.fps = fps
        self.device = device
        self.env_fps = 50
    ...
    def process_motions(self, dof_names):
        ...
        # self.motion_end_effector_pos[:, :, 2] += 0.05
        # self.motion_box_pos[:, 2] += 0.05
        # self.motion_box_pos_global[:, 2] += 0.05
        ...
        # print(torch.norm(self.motion_base_lin_vel[:, :2], dim=-1))
