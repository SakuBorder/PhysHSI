...
class MotionLib:
    def __init__(self, motion_file, mapping_file, dof_names, fps, device, window_length, ratio_random_range):
        self.fps = fps
        self.device = device
        self.env_fps = 50
        self.window_length = window_length
        self.ratio_random_range = ratio_random_range
        self.end_effector_name = ["left_palm_link", "right_palm_link", "left_ankle_pitch_link", "right_ankle_pitch_link", "keyframe_head_joint"]
    ...
    def process_motions(self, dof_names):
        ...
        print(torch.mean(torch.norm(self.motion_base_lin_vel[:, :2], dim=-1)))
    ...
    def get_expert_obs(self, batch_size):
        ...
        for i in range(self.window_length):
            ...
            amp_obs = torch.cat([amp_obs, base_height_next, motion_dof_pos_next, end_effector_pos_next, base_lin_vel_next, base_ang_vel_next, base_rot_next], dim=-1).view(batch_size, -1)
