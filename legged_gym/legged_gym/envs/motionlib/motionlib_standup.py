    ...
    def process_motions(self, dof_names):
        ...
        for skill in self.skills:
            for i, traj in enumerate(self.motion_data[skill]):
                ...
                if skill == "standup":
                    traj_bias = traj["base_height_bias"]
                    ...
                    self.motion_base_height[start:end] = self.motion_base_pos[start:end, 2:3].clone()
                ...
                if "chair_pos" in traj.keys():
                    ...
                    self.motion_chair_pos_global[start:end] = self.motion_base_pos[start:start+1].repeat(end-start, 1) # chose base_pos of last frame as the chair_pos
                    self.motion_chair_pos_global[start:end, 1] += 0.05
                    self.motion_chair_pos_global[start:end, 2] -= 0.18
                    self.motion_chair_pos[start:end] = self.motion_chair_pos_global[start:end] - self.motion_base_pos[start:end]
                    self.motion_chair_quat[start:end] = torch.tensor([0., 0., -0.7017, 0.7017], dtype=torch.float, device=self.device).view(1, 4).repeat(end-start, 1)
                else:
